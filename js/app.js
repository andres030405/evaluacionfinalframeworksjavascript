/*Alternación del título*/
/*Con la siguiente función hacemos que el titulo se anime*/
function cambiocolortitulo(selector){
    $(selector).animate({
        opacity: '1',
    }, {
        step: function (){
    ...
    .animate({
        ...
        step: function(){
            $(this).css('color', 'yellow');
            cambiocolortitulo('h1.main-titulo');
        },
        queue: true
    });
}
...
/*Funcion para obtener filas de dulces o columnas*/
function obtenerarraydedulces(arrayType, index) {
...
function candyRows(index){
    var candyRow = obtenerarraydedulces('rows', index);
    return candyRow;
}

/*Arreglos de columnas*/
function candyColumns(index){
    var candyColumn = obtenerarraydedulces('columns');
    return candyColumn[index];
}
/*Validación si hay dulces que se eliminan en una columna*/
...
/*Contador y mostrador de la puntuación*/
...
/*Funcion para elegir las imagenes y ponerlas*/
...
function fillBoard() {
	...
	column.each(function () {
		...
		for (var i = 0; i < agrega; i++) {
			var candyType = obtenernumerosaleatorios(1, 5);
			if (i === 0 && candys < 1) {
...
/*Interacción del usuario con el elemento caramelo es drag an drop(agarrar y soltar)*/
/*Efectos de movimiento entre los caramelos */
